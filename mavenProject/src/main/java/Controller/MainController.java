...
public class MainController {
	...
	public static void NewDemande(String title, String explication, String demandeur, String jour, String ville) throws BadLengthException{
		...
		try {
			...
			//insérer l'avis dans la database
			String commande = "INSERT into DemandeAide VALUES ('"+title+"','"+explication+"','"+demandeur+"','EN_ATTENTE','"+jour+"','"+ville+"','');";
		    state.executeUpdate(commande);
	...
	public static String getInfoOfDemand(String title, String info) throws UnexistingInfoException, UnexistingDemandException {
		String information = "" ;
		int column ;
		String url = "jdbc:mysql://srv-bdens.insa-toulouse.fr:3306/projet_gei_014";
		String user = "projet_gei_014";
		...
		try {
		    ...
		    ResultSet res = state.executeQuery("SELECT * FROM DemandeAide WHERE title = '"+title+"';");
		    if (!(res.next())) {
	...
	public static void setStatusOfDemand(String title, StatutDemande nvStatut) throws UnexistingDemandException {
		String url = "jdbc:mysql://srv-bdens.insa-toulouse.fr:3306/projet_gei_014";
		String user = "projet_gei_014";
		String passwd = "Rei4wie9";
		...
		try {
		    ...
		    if (!(res.next())) {
			    ...
			    String commande = "UPDATE DemandeAide SET state = '"+nvStatut+"' WHERE title = '"+title+"' ;";
		    	state.executeUpdate(commande);
		    }
	...
	public static ArrayList<String> getListOfDemands(StatutDemande statut){
		...
		try {
		    ...
		    while (res.next()) {
		    	 listTitles.add(res.getString(1)); 
		    } 
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(getInfoOfDemand("Besoin daide 2", "explication"));
			System.out.println(getInfoOfDemand("Besoin daide 4", "voisin"));
		...
		} catch (UnexistingDemandException exc4) {
